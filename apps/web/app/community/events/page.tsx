...
export default function EventsPage() {
  ...
  const createHref = "/events/create"
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-screen-xl space-y-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <Button asChild>
          <Link href={createHref}>+ Host an Event</Link>
        </Button>
      </div>
                  ...
                  <Card key={e.id} className="bg-gradient-to-br from-primary/5 to-transparent border-primary/20 hover:shadow-md transition-all">
                    <CardHeader>
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <Badge className={`text-xs border ${getCategoryClass(e.category)} bg-transparent`}>{e.category || "General"}</Badge>
                      </div>
                    </CardHeader>
